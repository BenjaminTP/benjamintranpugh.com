...
4. **Banned characters: the em dash and the middot.** Both read as AI-generated and are forbidden everywhere in `src/` and `public/`: copy, captions, tags, telemetry, page titles, alt text, code comments, and embedded files in `public/sims/`. This includes the HTML entities `&mdash;` and `&middot;`. Replace them:
   - list or field separator, and captions -> ` / ` (`CLASS-C ROAD / PASSIVE`)
   - page `<title>` -> ` | ` (`Active Suspension | Benjamin Tran-Pugh`)
   - label then explanation -> `:` (`GLOBAL STYLES: fonts, base elements`)
   - mid-sentence aside -> a comma, or split into two sentences
   - SI units -> a plain space (`1783 N s/m`)
   - anything mathematical -> real LaTeX (`$...$`), never a typed symbol

   Verify before every commit, expecting zero hits in text files:
   `grep -rnI "—\|·\|&mdash;\|&middot;" src public`
5. **Cheap edits stay cheap.** Read only the files the task needs. Content tasks live in ONE `.mdx` file: do not open components or styles for them. Never rewrite a whole file to change one field. If the user's request is a one-word change documented in `EDITING.md`, make the one-word change (and you may point them to the EDITING.md recipe for next time).
...
| Resume | replace `public/resume.pdf` only. The page shows an auto-rendered image preview of the PDF (built by `scripts/render-resume.mjs` via an integration in `astro.config.mjs`); `resume-page-*.png` are generated + gitignored. Do not hand-type resume content. |
...
Project frontmatter fields (`title, order, featured, summary, telemetry, tags, cover, github, writeup`) are validated by `src/content.config.ts`. `order` controls grid position; `<Figure src caption align="left|right|full" />` places images; missing images auto-render as placeholders.
